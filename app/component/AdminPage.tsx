...
const AdminDashboard: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      switch(activeTab) {
        case 'users':
          const usersResponse = await axios.get('https://library-management-backend-nweb.onrender.com/admin/view-all-users', config);
          console.log("users", usersResponse.data.response)
          setUsers(usersResponse.data.response);
          break;
        case 'moderators':
          const moderatorsResponse = await axios.get('https://library-management-backend-nweb.onrender.com/admin/view-all-moderators', config);
          setModerators(moderatorsResponse.data.response);
          break;
        case 'books':
          const booksResponse = await axios.get('https://library-management-backend-nweb.onrender.com/api/view-books', config);
  ...
  const handleDeleteUser = async (userId: string) => {
    ...
    try {
      const token = localStorage.getItem('access_token');
      await axios.delete(`https://library-management-backend-nweb.onrender.com/admin/delete-user/${userId}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const handleDeleteModerator = async (moderatorId: string) => {
    ...
    try {
      const token = localStorage.getItem('access_token');
      await axios.delete(`https://library-management-backend-nweb.onrender.com/admin/delete-moderator/${moderatorId}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const handleDeleteBook = async (bookId: string) => {
    ...
    try {
      const token = localStorage.getItem('access_token');
      await axios.delete(`https://library-management-backend-nweb.onrender.com/admin/delete-book/${bookId}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
